        ...
        <div class="dialog-container">
            ...
            <div class="dialog-content">
                ...
                <div v-if="currentTab === 'level'">
                    ...
                    <!-- Weapon slots: DNA characters carry a melee AND a ranged weapon -->
                    <div class="weapon-slots">
                        <div class="weapon-slot">
                            <span class="weapon-slot-label">Melee</span>
                            <span class="weapon-slot-value">{{ formatWeaponType(character?.weapon_type) }}</span>
                        </div>
                        <div class="weapon-slot">
                            <span class="weapon-slot-label">Ranged</span>
                            <span class="weapon-slot-value">{{ formatWeaponType(character?.weapon_type_ranged) }}</span>
                        </div>
                    </div>
                ...
                <div v-if="currentTab === 'skills'">
                    <!-- Levelable skills: Lv1 → Lv10, all three share one cost table -->
                    <div class="subsection-title">Skills (Lv1 → Lv10)</div>
                    ...
                    <!-- One-time unlocks: 0 = locked, 1 = unlocked. No level range. -->
                    <div class="subsection-title">Talent &amp; Passive Unlocks (one-time)</div>
                    <div v-for="(skill, index) in passiveSkills" :key="'passive-' + index" class="skill-row">
                        <div class="skill-badge badge-unlock">U</div>
                        <span class="skill-name">{{ skill.label }}</span>
                        <div class="skill-controls">
                            <label class="unlock-toggle">
                                <input
                                    type="checkbox"
                                    :checked="getPassiveValue(skill, 'current') > 0"
                                    @change="onUnlockToggle(skill, 'current', $event.target.checked)"
                                />
                                <span>Owned</span>
                            </label>
                            <span class="skill-arrow">→</span>
                            <label class="unlock-toggle">
                                <input
                                    type="checkbox"
                                    :checked="getPassiveValue(skill, 'target') > 0"
                                    @change="onUnlockToggle(skill, 'target', $event.target.checked)"
                                />
                                <span>Want</span>
                            </label>
                        </div>
...
<script setup>
/**
 * Duet Night Abyss character dialog.
 *
 * Same two-axis shape as the WutheringWaves dialog (levelable skills +
 * passives), with one behavioural difference: DNA's six talent/passive entries
 * are one-time unlocks rather than levels, so they render as Owned/Want
 * checkboxes that write 0/1 into the same current_level/target_level fields the
 * progression engine already reads.
 */
...
// weapon_type ids are snake_case; show them as words
...
/**
 * Unlocks are binary. Checking "Owned" implies the goal is met, so it drags
 * "Want" along; unchecking "Want" drags "Owned" down for the same reason the
 * level selects clamp each other.
 */
const onUnlockToggle = (skill, type, checked) => {
    const currentKey = `${skill.model_value}_current_level`;
    const targetKey = `${skill.model_value}_target_level`;
    ...
    if (type === 'current') {
        ...
        if (value > (props.settings.passiveSkills[targetKey] ?? 0)) {
            props.settings.passiveSkills[targetKey] = value;
        }
    } else {
        props.settings.passiveSkills[targetKey] = value;
        if (value < (props.settings.passiveSkills[currentKey] ?? 0)) {
            props.settings.passiveSkills[currentKey] = value;
        }
    }
...
.badge-unlock { background: #9b59b6; }
