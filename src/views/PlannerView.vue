<template>
  <div>
    ...
    <WeaponDialog v-if="dialogVisible && dialogType === 'weapon' && selectedWeapon" :visible="dialogVisible"
      ...
      @updateweapon="updateweapon" @completeTab="handleCompleteTab" />
  </div>
</template>
...
const handleCompleteTab = ({ tabType, materials, settingsUpdate }) => {
  ...
  Object.entries(materials || {}).forEach(([k, v]) => {
    if (k !== 'processed' && v > 0) filteredMaterials[k] = v;
  });

  if (Object.keys(filteredMaterials).length === 0) {
    toast.info('No materials needed', { position: 'bottom-center', autoClose: 2000 });
    return;
  }

  const validation = validateMaterialsWithSynthesis(filteredMaterials, inventory.value, tieredMaterials.value);
  ...
  let confirmMessage = `Complete ${tabType} for ${entityName}?`;
  if (validation.synthesisNeeded.length > 0) {
    confirmMessage += '\n\nAuto-synthesis will be performed:';
    validation.synthesisNeeded.forEach(syn => {
      confirmMessage += `\n• ${syn.materialName} (from lower tier)`;
    });
  }
  ...
  if (validation.synthesisNeeded.length > 0) {
    performSynthesis(validation.synthesisNeeded);
    toast.info('Materials synthesized automatically', { position: 'bottom-center', autoClose: 2000 });
  }

  const materialsToRemove = [];
  Object.entries(filteredMaterials).forEach(([materialId, quantity]) => {
    if (isExpCategory(materialId)) {
      let remainingExp = quantity;
      const expMats = getExpMaterials(materialId);
      for (const item of expMats) {
        const available = inventory.value[item.id] || 0;
        const toUse = Math.min(available, Math.ceil(remainingExp / item.value));
        if (toUse > 0) {
          materialsToRemove.push({ materialId: item.id, quantity: toUse });
          remainingExp -= toUse * item.value;
        }
  ...
  const tabLabel = tabType === 'level' ? 'Level' : 'Skills';
  toast.success(`${tabLabel} completed for ${entityName}!`, {
    position: 'bottom-center',
    autoClose: 3000,
    theme: 'dark',
  });
  ...
  setTimeout(() => {
    const currentSett = type === 'character'
      ? plannerStore.characterSettings[id]
      : plannerStore.weaponSettings[id];

    const updatedSettings = { ...currentSett, ...settingsUpdate };
