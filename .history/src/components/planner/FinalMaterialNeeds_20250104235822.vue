<template>
    <div class="final-material-needs">
        ...
        <div class="final-container">
            ...
            v-for="(category, categoryName) in categorizedMaterials"
            :key="categoryName"
            >
...
const groupMaterialsByCategoryAndSubCategory = (data) => {
    ...
    for (const [materialId, details] of Object.entries(materials)) {
        ...
        const need = details.need || 0;
              ...
              // subcategory 초기화
              if (!groupedMaterialsMap[subCategory]) {
            groupedMaterialsMap[subCategory] = {
                id: subCategory,
                name: subCategory,
                category: category,
        ...
        // task 데이터 추가
        groupedMaterialsMap[subCategory].task.push({
            id: materialId,
            name: name,
            need: need,
            owned: owned,
    ...
    // 객체를 배열로 변환
    const groupedMaterials = Object.values(groupedMaterialsMap);
...
const GetRateValueForCategory = (data) => {
    ...
    categorizedMaterials.value.forEach((material) => {
        // subCategory 매칭 확인
        if (material.category === data.category) {
