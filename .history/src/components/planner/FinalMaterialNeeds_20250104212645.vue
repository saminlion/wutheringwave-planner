...
const groupMaterialsByCategoryAndSubCategory = (data) => {
    ...
    for (const [materialId, details] of Object.entries(materials)) {
        ...
        const owned = details.owned;
        const synthesize = details.synthesize;
        const need = details.need;
        ...
        // 서브카테고리 초기화
        if (!groupedMaterials[category].subCategories[subCategory]) {
            groupedMaterials[category].subCategories[subCategory] = {
                name: subCategory,
                tasks: []
            };
        }
        ...
        // task 데이터 추가
        groupedMaterials[category].subCategories[subCategory].tasks.push({
            materialId,
            owned,
            synthesize,
            need
