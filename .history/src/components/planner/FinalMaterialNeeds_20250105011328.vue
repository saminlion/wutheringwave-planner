    ...
    Object.entries(data.subCategories).forEach(([subcategoryName, subcategoryData]) => {
        ...
        if (!unobtainable) {
            ...
            if (categoryName === "forgery" || categoryName === "common") {
                ...
                //calculatePlayerExp(expNeeded, expMaterialTypeStructure, ownedMaterials)
...
.material-card {
    ...
    padding: 16px; /* 여백을 증가 */
    width: 80px; /* 카드의 너비를 증가 */
    height: 120px; /* 카드의 높이를 증가 */
    box-shadow: 0 2px 6px rgba(0, 0, 0, 0.15); /* 그림자 강조 */
}

.material-icon {
    width: 50px; /* 이미지 크기 증가 */
    height: 50px; /* 이미지 크기 증가 */
    margin-bottom: 8px; /* 텍스트와 이미지 간격 증가 */
}
