    ...
    <div class="final-material-needs">
        ...
        <div class="final-container">
            <div class="category-card" v-for="(category, categoryName) in categorizedMaterials" :key="categoryName">
                ...
                <!-- 필요 정보 표시 -->
                <span class="material-set" v-if="totalExpNeed > 0">
                    필요: {{ totalExpNeed }}
                </span>
                                <li class="material-card" v-for="(expDetails, id) in player_exp_material" :key="id">
                                    <div class="material-info">
                                        ...
                                        <span class="material-quantity">
                                            보유: {{ inventoryStore.getMaterialQuantity(id) }}
                                        </span>
                                    </div>
                                </li>
                            </template>
                            ...
                            <template
                                v-else-if="category.name !== 'common' && category.name !== 'forgery'">
                                <li class="material-card" v-for="task in subCategory.task" :key="task.id">
                                    <div class="material-info">
                                        ...
                                        <span class="material-quantity">필요: {{ task.need }}</span>
                                        <span class="material-quantity">보유: {{ task.owned }}</span>
                                    </div>
                                </li>
                            </template>
                            ...
                            <template v-else>
                                <li class="material-card" v-for="task in subCategory.task" :key="task.id">
                                    <div class="material-info">
                                        ...
                                        <span class="material-quantity">필요: {{ task.need }}</span>
                                        <span class="material-quantity">보유: {{ task.owned }}</span>
                                        <span class="material-quantity">합성: {{ task.synthesize }}</span>
                                    </div>
                                </li>
                            </template>
                        </ul>
...
const totalExpNeed = computed(() => {
    if (category.name === "player_exp" || category.name === "weapon_exp") {
        let totalNeed = 0;

        Object.entries(player_exp_material).forEach(([id, expValue]) => {
            const ownedQty = inventoryStore.getMaterialQuantity(id);
            totalNeed += ownedQty * expValue;
        });

        return Math.max(0, requiredExp - totalNeed); // 필요한 경험치에서 보유한 경험치를 뺌
    }
    return 0; // 다른 카테고리는 필요하지 않음
});
...
.category-card {
    margin-bottom: 24px; /* 카테고리 간격 */
    padding: 16px; /* 내부 여백 */
    background-color: #f7f9fc; /* 배경색 */
    border-radius: 8px; /* 둥근 모서리 */
    box-shadow: 0 2px 6px rgba(0, 0, 0, 0.1); /* 그림자 */
}
...
.category-title {
    ...
    font-weight: bold;
    color: #2c3e50;
    margin-bottom: 12px; /* 카테고리 타이틀 아래 간격 */
}
...
.materials-grid {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); /* 카드 크기 및 균등한 열 배치 */
    gap: 16px; /* 카드 간 간격 */
    margin-top: 12px;
    list-style: none;
    padding: 0;
    justify-content: center;
    align-items: start; /* 카드의 상단 정렬 */
}
...
.material-card {
    ...
    box-shadow: 0 2px 6px rgba(0, 0, 0, 0.15); /* 그림자 추가 */
}
...
.material-quantity {
    color: #27ae60; /* 텍스트 강조 색상 */
    font-size: 16px; /* 텍스트 크기 */
    font-weight: bold;
    margin-top: 4px; /* 텍스트 간격 */
}
