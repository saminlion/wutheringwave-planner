...
const updateSkillLevel = (skill, type, delta) => {
    ...
    const newValue =  props.settings.activeSkills[key] + delta;
    if (newValue >= 1 && newValue <= 10) { // 최대값 10
        props.settings.activeSkills[key] = newValue;
        updateCharacter();
    }
};
...
const validateSkillLevel = (skill, type) => {
    const key = `${skill.model_value}_${type}_level`;
    const value =  props.settings.activeSkills[key];
    if (value < 1) {
        props.settings.activeSkills[key] = 1; // 최소값
    } else if (value > 10) {
        props.settings.activeSkills[key] = 10; // 최대값
    }
    updateCharacter();
};
