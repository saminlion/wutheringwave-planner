...
export const processMaterials = (materials, key, value, characterInfo) => {
  if (['common', 'player_exp', 'weapon_exp', 'forgery'].includes(key)) {
    ...
    const gameId = characterInfo[key]; // 여기서 game_id를 가져옴
    if (!gameId) {
      console.warn(`Missing game_id for key "${key}" in character info.`);
      return;
    }
    const material = findMaterial(key, gameId, null, true); // game_id 기반 검색
    if (material) {
      const materialKey = getMaterialField(material, 'label');
      if (materialKey) {
      ...
      console.warn(`Material not found for key: ${key}, game_id: ${gameId}`);
    }
  } else if (key !== 'level') {
