...
export const calculateTotalMaterialNeeds = (totalNeeds, inventory, synthesizableTypes = ['common', 'forgery']) => {
    ...
    synthesizableTypes.forEach((type) => {
      // 'type'에 맞는 재료들을 전부 가져온 후, totalNeeds에 있는 재료만 필터링
      const materials = Object.values(inventoryItem[type])
        ...
        .sort((a, b) => a.rarity - b.rarity); // rarity 기준 정렬
  ...
  // 합성 처리
      for (let i = 0; i < materials.length - 1; i++) {
        ...
        const neeedQty = updatedNeeds[currentMaterial.game_id] || 0; // 아이템 필요량
        const ownedQty = inventory[currentMaterial.game_id] || 0; // 인벤토리에서 해당 아이템의 보유 수량 확인

        console.log(`inventory: ${inventory[currentMaterial.game_id]} / currentMaterial.game_id: ${currentMaterial.game_id}`);
        console.log(`neeedQty: ${neeedQty} / ownedQty: ${ownedQty}`);

        const updateQty = ownedQty - neeedQty;

        // 합성 가능 수량 계산
        const synthesizedQty = Math.min(
          Math.floor(updateQty / synthesisCost), // 필요한 재료의 수량에 맞춰 합성 가능 수량 계산
          updatedNeeds[nextMaterial.game_id] || Infinity, // 다음 재료의 필요 수량이 충분히 있어야 합성 가능
          updateQty // 인벤토리에서의 수량에 따라 합성할 수 있는 수량 제한
        );

        console.log(`synthesizedQty: ${synthesizedQty} / nextMaterial.game_id: ${nextMaterial.game_id}`);

        if (synthesizedQty > 0) {
          // 합성 후 업데이트
          updatedNeeds[currentMaterial.game_id] -= synthesizedQty;
