...
export const calculateTotalMaterialNeeds = (totalNeeds, synthesizableTypes = ['common', 'forgery'], inventory) => {
    ...
    synthesizableTypes.forEach((type) => {
        ...
        for (let i = 0; i < materials.length - 1; i++) {
            ...
            updateMaterial[currentMaterial.game_id].owned = inventory[currentMaterial.game_id] || 0;
        ...
        Object.values(updateMaterial).forEach(items => {
            items.forEach(item=>{
                const task = item;

                const getCurrentTotal = task.owned + task.synthesize;

                task.acutalNeed = Math.ceil(Math.max(0, (getCurrentTotal - task.need) * -1));
            });
        });
    });
