...
const findAllMatchingSubCategories = (inventoryItem, shortages) => {
    ...
    // Return an array of SubCategory for all matched items
    return matchedItems.map((item) => item.SubCategory);
};
...
export const performItemSynthesisWithNeeds = (inventory, tieredMaterials, shortages) => {
    ...
    for (const category in tieredMaterials) {
        ...
        const materialDataw = findAllMatchingSubCategories(inventoryItem, shortages);
