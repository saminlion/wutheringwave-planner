...
export const calculateMaterials = (inventory, tieredMaterials, shortages) => {
    ...
    // Ensure shortages and inventory are valid objects
    if (!shortages || typeof shortages !== 'object') {
        console.error("[Error] Shortages is undefined or invalid. Initializing as empty object.");
        shortages = {};
    }

    if (!inventory || typeof inventory !== 'object') {
        console.error("[Error] Inventory is undefined or invalid. Initializing as empty object.");
        inventory = {};
    }

    // Perform forward synthesis
    const forwardResult = performItemSynthesisWithNeeds(inventory, tieredMaterials, shortages);
    if (!forwardResult || !forwardResult.updatedInventory) {
        console.error("[Error] Forward synthesis returned invalid result.");
    ...
    let finalNeeds;
    try {
        finalNeeds = backwardConversion(forwardResult.updatedInventory, tieredMaterials, shortages);
        if (!finalNeeds || typeof finalNeeds !== 'object') {
            throw new Error("Invalid result from backward conversion.");
        }
    } catch (e) {
        console.error("[Error] Backward Conversion failed:", e);
        return { final_inventory: {}, synthesis_results: {}, final_needs: {} };
    }
