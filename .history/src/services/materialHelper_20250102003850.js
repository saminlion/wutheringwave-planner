...
export const calculateTotalMaterialNeeds = (totalNeeds, synthesizableTypes = ['common', 'forgery']) => {
    ...
    synthesizableTypes.forEach((type) => {
      const materials = Object.values(inventoryItem[type]).sort((a, b) => a.rarity - b.rarity); // rarity 기준 정렬
 ...
 */
const isSynthesizable = (materialId, synthesizableTypes) => {
    return synthesizableTypes.some((type) => {
      const material = findMaterial(type, materialId, null, true); // game_id 기준으로 검색
