...
/**
 * Finds material by type and identifier (id or SubCategory).
 *
 * @param {string} type - The type of material (e.g., "common", "ascension", "boss").
 * @param {string} identifier - The id (nameKey) or SubCategory of the material.
 * @param {number|null} rarity - The rarity of the material (optional).
 * @param {boolean} useId - Whether to use id (nameKey) for matching.
...
export const findMaterial = (type, identifier, rarity = null, useId = false) => {
    ...
    if (useId) {
      ...
      if (!materialData) {
        console.warn(`Material with id "${identifier}" not found in type "${type}".`);
        return null;
      }
      return materialData;
    } else {
...
export const processMaterials = (materials, key, value, characterInfo) => {
    if (['common', 'player_exp', 'weapon_exp', 'forgery'].includes(key)) {
      const [qty, tier] = value;
      const material = findMaterial(key, characterInfo.common, tier);
      if (material) {
        const materialKey = getMaterialField(material, 'label');
        if (materialKey) {
          materials[materialKey] = (materials[materialKey] || 0) + qty;
        }
      }
    } else if (['ascension', 'boss', 'weeklyBoss', 'shell_credit'].includes(key)) {
      const nameKey = characterInfo[key];
      const material = findMaterial(key, nameKey, null, true);
      if (material) {
        const materialKey = getMaterialField(material, 'label');
        if (materialKey) {
