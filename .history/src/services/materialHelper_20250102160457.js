...
export const performItemSynthesisWithNeeds = (inventory, tieredMaterials, shortages) => {
    ...
    for (const category in tieredMaterials) {
        ...
        const materialDataw = Object.values(inventoryItem).find(
            (item) =>
                shortages.hasOwnProperty(String(item.game_id)) && // shortages에 game_id가 있는지 확인
                item.category === "subcategory" // 카테고리가 subcategory인지 확인
        );
