...
export const backwardConversion = (inventory, tieredMaterials, shortages) => {
    for (const category in tieredMaterials) {
        ...
        for (const rarity of rarities) {
            ...
            // Handle shortages for the current material
            if (materialGameId in shortages) {
                const required = shortages[materialGameId];
                inventory[materialGameId] = Math.max(0, (inventory[materialGameId] || 0) - required);
            }

            // Convert surplus to lower rarity if applicable
