...
export const calculateTotalMaterialNeeds = (totalNeeds, inventory, synthesizableTypes = ['common', 'forgery']) => {
    ...
    synthesizableTypes.forEach((type) => {
      ...
      for (let i = 0; i < materials.length - 1; i++) {
        ...
        const ownedQty = inventory.value[currentMaterial.game_id] || 0; // 인벤토리에서 해당 아이템의 보유 수량 확인
  
        // 합성 가능 수량 계산 (최대 합성 수량은 인벤토리에서 가지고 있는 수량에 따라 제한됨)
        const synthesizedQty = Math.min(
          Math.floor(currentQty / synthesisCost), // 필요한 수량 기준
          updatedNeeds[nextMaterial.game_id] || Infinity, // 다음 재료 수량 기준
          Math.floor(ownedQty / synthesisCost) // 인벤토리에서 보유한 수량 기준
        );
