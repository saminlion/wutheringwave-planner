...
export const performItemSynthesisWithNeeds = (inventory, tieredMaterials, shortages) => {
    ...
    for (const category in tieredMaterials) {
        ...
        for (const rarity of Object.keys(materialData).sort((a, b) => a - b)) {
            ...
            const currentMaterialName = currentLevel.name;
            const nextMaterialName = nextLevel.name;

            const currentCount = inventory[currentMaterialName] || 0;
            const requiredCount = currentLevel.synthesizable?.count || 1;
            const needed = shortages[currentMaterialName] || 0;
            ...
            if (synthesisQty > 0) {
                inventory[currentMaterialName] -= synthesisQty * requiredCount;
                inventory[nextMaterialName] = (inventory[nextMaterialName] || 0) + synthesisQty;
                synthesisResults[currentMaterialName] = {
...
export const backwardConversion = (inventory, tieredMaterials, shortages) => {
    for (const category in tieredMaterials) {
        ...
        for (const rarity of rarities) {
            ...
            const nextRarity = rarities[+rarity - 1];
            const nextMaterial = materialData[nextRarity];
            const nextMaterialName = nextMaterial?.name;

            if (materialName in shortages) {
                const required = shortages[materialName];
                inventory[materialName] = Math.max(0, inventory[materialName] - required);
            }

            if (nextMaterialName) {
                const surplus = inventory[materialName] || 0;
                const converted = surplus * 3;
                inventory[materialName] = 0;
                inventory[nextMaterialName] = (inventory[nextMaterialName] || 0) + converted;
            }
        }
    }
...
export const calculateMaterials = (inventory, tieredMaterials, shortages) => {
    ...
    const finalInventory = backwardConversion(forwardResult.updatedInventory, tieredMaterials, shortages);
    return { finalInventory, synthesisResults: forwardResult.synthesisResults };
};