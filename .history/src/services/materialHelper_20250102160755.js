...
const findItemSubCategory = (inventoryItem, shortages) => {
    ...
    // Find the first item where game_id matches any key in shortages
    const matchedItem = allItems.find((item) =>
        shortages.hasOwnProperty(String(item.game_id)) // Match game_id with shortages keys
    );

    // Return the SubCategory if the item is found, else return null
    return matchedItem ? matchedItem.SubCategory : null;
};
...
export const performItemSynthesisWithNeeds = (inventory, tieredMaterials, shortages) => {
    ...
    for (const category in tieredMaterials) {
        ...
        const materialDataw = findItemSubCategory(inventoryItem, shortages);

        console.log(`[Debug] Initial Material Dataw : ${JSON.stringify(materialDataw)}`);
