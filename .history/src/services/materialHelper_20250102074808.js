 ...
 */
export const calculateTotalMaterialNeeds = (totalNeeds, synthesizableTypes = ['common', 'forgery'], inventory) => {
    // totalNeeds를 복사하고, 각 항목에 need, owned, synthesize를 추가
    const updateMaterial = Object.keys(totalNeeds).reduce((acc, materialId) => {
        acc[materialId] = {
            need: totalNeeds[materialId],  // 기존 값
            acutalNeed: 0,
            owned: inventory[materialId] || 0,  // 기본값
            customOwned: 0,                // 기본값
            synthesize: 0                  // 기본값
        };
        return acc;
    ...
    synthesizableTypes.forEach((type) => {
        ...
        for (let i = 0; i < materials.length - 1; i++) {
            ...
            updateMaterial[currentMaterial.game_id].customOwned =  updateMaterial[currentMaterial.game_id].customOwned || inventory[currentMaterial.game_id];

            //updateMaterial[currentMaterial.game_id].customOwned = updateMaterial[currentMaterial.game_id].owned;

            const currentCustomOwned = updateMaterial[currentMaterial.game_id].customOwned;
            const currentNeed = updateMaterial[currentMaterial.game_id].need;

            // 합성 가능 수량 계산
            const synthesizedQty = Math.max(
                0,
                Math.floor(( currentCustomOwned -  currentNeed) / synthesisCost)
            );

            console.log(`Updated: ${updateMaterial[currentMaterial.game_id].owned} /${currentNeed}/ ${synthesizedQty}`);

            if (nextMaterial != null) {
                // 합성 후 업데이트
                updateMaterial[nextMaterial.game_id].customOwned += updateMaterial[nextMaterial.game_id].owned + synthesizedQty;
                
                updateMaterial[nextMaterial.game_id].synthesize = Math.max(0, Math.min(updateMaterial[nextMaterial.game_id].need - updateMaterial[nextMaterial.game_id].owned, synthesizedQty));

                console.log(`Updated Inventory after synthesis currentMaterial.game_id: 
            / ${currentMaterial.game_id} = ${updateMaterial[currentMaterial.game_id].customOwned} /
             ${nextMaterial.game_id}: ${updateMaterial[nextMaterial.game_id].customOwned} /
             synthesizedQty: ${synthesizedQty}
            `
                );
            }
            else
            {
                updateMaterial[currentMaterial.game_id].acutalNeed = updateMaterial[currentMaterial.game_id].customOwned - updateMaterial[currentMaterial.game_id].need;
            }
        }

        for (let i = materials.length - 1; i > 0; i--) {
            const currentMaterial = materials[i];
            const prevMaterial = materials[i - 1];  // 현재 작업의 이전 작업을 가져옴

            const prevLeftover = Math.max(0, updateMaterial[prevMaterial.game_id].owned - updateMaterial[prevMaterial.game_id].need);
            const currentMissing = updateMaterial[currentMaterial.game_id].synthesize * 3 - prevLeftover;

            updateMaterial[prevMaterial.game_id].synthesize += Math.max(0, currentMissing);

        }
    });

    // 이제 actualNeed만 반환
    const finalNeeds = Object.keys(updateMaterial).reduce((acc, materialId) => {
        acc[materialId] = updateMaterial[materialId].acutalNeed; // materialId와 actualNeed만 반환
        return acc;
    }, {});
