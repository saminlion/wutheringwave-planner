...
export const calculateTotalMaterialNeeds = (totalNeeds, inventory, synthesizableTypes = ['common', 'forgery']) => {
    ...
    synthesizableTypes.forEach((type) => {
        ...
        for (let i = 0; i < materials.length - 1; i++) {
            ...
            const needQty = updatedNeeds[currentMaterial.game_id] || 0; // 아이템 필요량
            ...
            // 합성 가능 수량 계산 (보유량에서 필요한 양만큼만 합성 가능)
            const synthesizableQty = Math.min(
                Math.floor(ownedQty / synthesisCost), // 필요한 재료의 수량에 맞춰 합성 가능 수량 계산
            ...
            if (synthesizableQty > 0) {
                // 합성 후 업데이트
                updatedNeeds[currentMaterial.game_id] -= synthesizableQty * synthesisCost;
                updatedNeeds[nextMaterial.game_id] = (updatedNeeds[nextMaterial.game_id] || 0) + synthesizableQty;
