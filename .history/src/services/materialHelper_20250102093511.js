...
/**
 * Fixes `processed` issue for correct data representation.
 */
export const cleanProcessedData = (materials) => {
    const cleanedMaterials = { ...materials };
    if (cleanedMaterials.processed instanceof Set) {
        delete cleanedMaterials.processed;
    }
    return cleanedMaterials;
};

/**
 * Main calculation function with forward and backward synthesis.
 */
export const calculateMaterials = (inventory, tieredMaterials, shortages) => {
    ...
    console.log("[Debug] Synthesis Results:", forwardResult.synthesisResults);
