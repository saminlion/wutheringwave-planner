...
export const calculateTotalMaterialNeeds = (totalNeeds, inventory, synthesizableTypes = ['common', 'forgery']) => {
    ...
    const updateOwneds = {...inventory};

    console.log('totalNeeds:', totalNeeds); // 전체 필요량 로그
    console.log('updateOwneds:', updateOwneds); // 전체 필요량 로그
    ...
    synthesizableTypes.forEach((type) => {
        ...
        for (let i = 0; i < materials.length - 1; i++) {
            ...
            const ownedQty = inventory[currentMaterial.game_id] || 0;

            console.log(`inventory: ${inventory[currentMaterial.game_id]} / currentMaterial.game_id: ${currentMaterial.game_id}`);
            console.log(`needQty: ${needQty} / ownedQty: ${ownedQty}`);

            // 합성 가능 수량 계산 (보유량에서 필요한 양만큼 합성 가능)
            const synthesizableQty = Math.min(
                Math.floor(ownedQty / synthesisCost), // 보유량을 합성 비용으로 나눠서 합성 가능한 수량 계산
                updatedNeeds[nextMaterial.game_id] || Infinity // 다음 재료의 필요 수량이 충분해야 합성 가능
            );

            console.log(`synthesizedQty: ${synthesizableQty} / nextMaterial.game_id: ${nextMaterial.game_id}`);

            if (synthesizableQty > 0) {
                // 합성 후 업데이트
                updatedNeeds[currentMaterial.game_id] -= synthesizableQty * synthesisCost;  // 현재 재료의 필요량 차감
                inventory[currentMaterial.game_id] -= synthesizableQty * synthesisCost;  // 현재 재료의 보유량 차감
                inventory[nextMaterial.game_id] = (inventory[nextMaterial.game_id] || 0) + synthesizableQty;  // 합성 후 새로운 재료 보유량 증가
                updatedNeeds[nextMaterial.game_id] = (updatedNeeds[nextMaterial.game_id] || 0) + synthesizableQty;  // 다음 재료의 필요량 업데이트

                console.log(`Updated Needs after synthesis: ${JSON.stringify(updatedNeeds, null, 2)}`);
                console.log(`Updated Inventory after synthesis: ${JSON.stringify(inventory, null, 2)}`);
            }
        }
    });
