...
export const calculateTotalMaterialNeeds = (totalNeeds, inventory, synthesizableTypes = ['common', 'forgery']) => {
    ...
    synthesizableTypes.forEach((type) => {
        ...
        const materials = Object.values(inventoryItem[type])
            ...
            .sort((a, b) => b.rarity - a.rarity); // rarity 기준으로 내림차순 정렬
        ...
        for (let i = 0; i < materials.length - 1; i++) {
            ...
            if (synthesizableQty > 0) {
                // 합성 후 업데이트
                updatedNeeds[currentMaterial.game_id] = synthesizableQty - ownedQty;  // 현재 재료의 필요량 차감
                updateOwneds[currentMaterial.game_id] -= synthesizableQty * synthesisCost;
                updateOwneds[nextMaterial.game_id] += synthesizableQty;  // 다음 재료의 필요량 업데이트
            }
        }
    });
