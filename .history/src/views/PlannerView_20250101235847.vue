...
const finalMaterialNeeds = computed(() => {
  ...
  goals.value.forEach((goal) => {
    ...
    Object.entries(filteredMaterials).forEach(([material, qty]) => {
      if (material === 'player_exp') {
        ...
        // Directly add for synthesis calculation
  ...
  // Final needs initialization
  const finalNeeds = {};

  // Subtract inventory from synthesizedNeeds and handle non-synthesizable materials
  Object.entries(synthesizedNeeds).forEach(([material, qty]) => {
    const ownedQty = ownedMaterials[material] || 0;
    finalNeeds[material] = Math.max(qty - ownedQty, 0); // Ensure no negative values
  });

  Object.entries(totalNeeds).forEach(([material, qty]) => {
    if (!synthesizableTypes.some((type) => material.startsWith(type))) {
      // Handle general materials not part of synthesis logic
      const ownedQty = ownedMaterials[material] || 0;
      finalNeeds[material] = Math.max(qty - ownedQty, 0); // Ensure no negative values
    }
  });
