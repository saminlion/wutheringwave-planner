...
import { findMaterial, getMaterialField, calculatePlayerExp, performItemSynthesisWithNeeds, cleanProcessedData, calculateMaterials } from '@/services/materialHelper';
...
const finalMaterialNeeds = computed(() => {
    ...
    goals.value.forEach((goal) => {
        const cleanedMaterials = cleanProcessedData(goal.materials || {});
        Object.entries(cleanedMaterials).forEach(([materialId, qty]) => {
    ...
    // Step 4: Combine synthesized needs and player EXP
    const finalNeeds = { ...final_needs };
