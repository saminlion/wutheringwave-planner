  ...
  <div>
    ...
    <div v-if="selectedCharacter">
      ...
      <div>
        <label>
          Current Level:
          <select v-model="characterSettings.char_current_level" @change="updateCharacter">
        ...
        <label>
          Target Level:
          <select v-model="characterSettings.char_target_level" @change="updateCharacter">
            <option v-for="item in levelItems" :key="item.value" :value="item.value">
              {{ item.label }}
            </option>
          </select>
...
import { levelItems, activeSkills, passiveSkills } from '../data/formCharactersNew'; // 새 데이터 구조 참조
...
export default {
  ...
  setup() {
    ...
    const updateCharacter = () => {
      plannerStore.updateCharacterSettings(selectedCharacter.value.game_id, characterSettings.value);
    };
