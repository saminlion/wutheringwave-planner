...
const finalMaterialNeeds = computed(() => {
  ...
  // totalNeeds를 합성 가능/불가능한 재료로 분리 및 player_exp 처리
  Object.entries(totalNeeds).forEach(([materialId, qty]) => {
    if (materialId === 'player_exp') {
      playerExpNeeds = qty; // player_exp는 별도로 처리
    } else {
      const isSynthesizable = synthesizableTypes.some((type) =>
        Object.values(ownedMaterials[type]).some((item) => item.game_id === parseInt(materialId))
      );
  ...
  Object.entries(playerExpResults).forEach(([materialId, { needed, owned }]) => {
    finalNeeds[materialId] = Math.max(needed - owned, 0); // player_exp 결과 추가
  });
