...
watch(
  ...
  () => {
    ...
    finalMaterialNeeds.value = calculateMaterialsNeeds(); // 실시간 업데이트
  },
  { deep: true}
);
...
const updateFinalMaterialNeeds = () => {
  finalMaterialNeeds.value = calculateMaterialsNeeds();
};

const calculateMaterialsNeeds = () => {
  const totalNeeds = {};
...
onMounted(() => {
  updateFinalMaterialNeeds(); // 초기 계산
});
</script>
