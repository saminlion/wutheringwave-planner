  ...
  <div>
    ...
    <div v-if="selectedCharacter">
      ...
      <div v-for="(skill, index) in activeSkills" :key="index">
        ...
        <input type="number" v-model="characterSettings[selectedCharacter.game_id][`${skill.model_value}_current_level`]" @change="updateCharacter" />
        <label>Target Level:</label>
        <input type="number" v-model="characterSettings[selectedCharacter.game_id][`${skill.model_value}_target_level`]" @change="updateCharacter" />
      </div>
      ...
      <div v-for="(tier, tierIndex) in passiveSkills" :key="tierIndex">
        ...
        <div v-for="(skill, skillIndex) in tier.data" :key="skillIndex">
          ...
          <input type="checkbox" v-model="characterSettings[selectedCharacter.game_id][skill.model_value]" @change="updateCharacter" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup() {
    ...
    const updateCharacter = () => {
      plannerStore.updateCharacterSettings(selectedCharacter.value.game_id, characterSettings.value[selectedCharacter.value.game_id]);
    };
