...
| **Weapons** | 무기 데이터 |
| **Materials** | 재료 데이터 |
| **FarmingRates** | 스태미나 소비율 (샘플 기록 → 평균 자동 계산) |
...
파밍 시 드랍량을 기록하면 평균이 자동 계산됩니다. sync 시 `farmingRates.json`으로 저장되어 Estimated Days 계산에 사용됩니다.

> forgery는 티어별 드랍을 각각 기록 → T2-equivalent로 자동 변환
> player_exp/weapon_exp는 아이템 개수로 기록 → EXP 합산 자동 계산

**열 구성:**

| Column | Name | 입력방식 | Description |
|--------|------|---------|-------------|
| A | category | 직접 입력 | 카테고리명 (고정 13행) |
| B | stamina | 직접 입력 | 1회 파밍 스태미나 소비 |
| C | unobtainable | 직접 입력 | TRUE = Estimated Days 미표시 |
| D | value_per_item | 직접 입력 | EXP 아이템만 입력 (forgery는 0) |
| E | sample_count | **자동** | 수식: `=COUNTA(G2:2)` |
| F | avg | **자동** | 수식: `=IFERROR(ROUND(AVERAGE(G2:2),2),0)` |
| G~ | run_1, run_2, ... | 직접 입력 | 파밍할 때마다 드랍량 기록 |

**초기 데이터 (13행 고정 입력):**

| category | stamina | unobtainable | value_per_item | sample_count | avg | run_1 | ... |
|----------|---------|--------------|----------------|--------------|-----|-------|-----|
| credit | 40 | FALSE | 0 | 수식 | 수식 | | |
| common | 0 | TRUE | | | | | |
| forgery_1 | 40 | FALSE | 0 | 수식 | 수식 | | |
| forgery_2 | 40 | FALSE | 0 | 수식 | 수식 | | |
| forgery_3 | 40 | FALSE | 0 | 수식 | 수식 | | |
| boss | 0 | TRUE | | | | | |
| weeklyBoss | 0 | TRUE | | | | | |
| player_exp_1 | 40 | FALSE | 1000 | 수식 | 수식 | | |
| player_exp_2 | 40 | FALSE | 5000 | 수식 | 수식 | | |
| player_exp_3 | 40 | FALSE | 20000 | 수식 | 수식 | | |
| weapon_exp_1 | 40 | FALSE | 500 | 수식 | 수식 | | |
| weapon_exp_2 | 40 | FALSE | 2500 | 수식 | 수식 | | |
| weapon_exp_3 | 40 | FALSE | 10000 | 수식 | 수식 | | |

**E, F열 수식 (unobtainable=TRUE 행 제외, 행 번호 맞게 조정):**
```
E2: =COUNTA(G2:2)
F2: =IFERROR(ROUND(AVERAGE(G2:2),2),0)
```

**sync 계산 로직:**
- `forgery`: T2-equivalent = forgery_1_avg ÷ 3 + forgery_2_avg + forgery_3_avg × 3
- `player_exp`: 총 EXP/run = (avg × 1000) + (avg × 5000) + (avg × 20000)
- `weapon_exp`: 총 EXP/run = (avg × 500) + (avg × 2500) + (avg × 10000)
- `credit`: drops = avg (크레딧/run)

**사용 방법:**
1. 파밍 1회 완료 → 해당 행의 G열 다음 빈 칸에 드랍량 기록
2. avg(F열)가 자동 갱신됨
3. GitHub Actions sync 실행 → `farmingRates.json` 업데이트
